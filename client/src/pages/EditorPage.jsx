// src/pages/EditorPage.jsx (ฉบับแก้ไข: Final Fix - เพิ่ม State PlaybackCells)
...
// 🎯 FIX 3: แก้ไข handlePlayToggle (ใช้ State และ Callback ที่ถูกต้อง)
  const handlePlayToggle = () => { 
    if(isPlaying){ 
        stopSong(); 
        setIsPlaying(false); 
        // 🚨 FIX 4: แก้ ReferenceError โดยการล้าง Highlight ที่ State นี้
        setPlaybackCells([]); 
    } else { 
        setIsPlaying(true); 
        const startIndex = (selectedCell.row * 8) + selectedCell.col; 
        
        // Callback function สำหรับ Engine เพื่ออัปเดตตำแหน่ง Highlight
